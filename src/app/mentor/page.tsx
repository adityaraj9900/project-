...
import { LayoutDashboard, Users, ClipboardCheck, FileText, LogOut, Menu } from "lucide-react";
import { cn } from "@/lib/utils";
import { Badge } from "@/components/ui/primitives";
import { MentorHome } from "@/components/mentor/mentor-home";
import { MentorInterns } from "@/components/mentor/mentor-interns";
import { MentorReview } from "@/components/mentor/mentor-review";

const NAV = [
  { key: "home", label: "Overview", icon: LayoutDashboard },
  { key: "interns", label: "My Interns", icon: Users },
      ...
      <aside className={cn("fixed inset-y-0 left-0 z-40 flex w-60 flex-col border-r border-[rgba(201,168,76,0.1)] bg-[#0A0A0A] transition-transform duration-300", sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0")}>
        <div className="flex items-center gap-3 border-b border-[rgba(201,168,76,0.1)] px-5 py-5">
          ...
          <div>
            ...
            <div className="text-[10px] uppercase tracking-[0.22em] text-[#C9A84C]">Mentor</div>
          </div>
        </div>
        ...
        <div className="border-t border-[rgba(201,168,76,0.1)] p-4">
          <div className="mb-3 flex items-center gap-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[rgba(201,168,76,0.25)] bg-[rgba(201,168,76,0.08)] text-sm font-bold text-[#C9A84C]">{session?.user?.name?.[0] ?? "M"}</div>
